...
from StatTools.analysis.dfa import dfa, dfa_worker, _detrend_segment, _fluctuation_function
...
def test_fluctuation_function():
    """Test _fluctuation_function"""
    f_q_s_sum = 100.0
    cycles_amount = 10
    degree = 2
    ...
    # Expected: ((1/10) * 100)^(1/2) = sqrt(10) ≈ 3.16
    expected = ((1.0 / cycles_amount) * f_q_s_sum) ** (1.0 / degree)
    assert F_s == pytest.approx(expected, rel=1e-10)
    assert F_s > 0


def test_fluctuation_function_edge_cases():
    """Test _fluctuation_function with edge cases"""
    # Single cycle
    F_s = _fluctuation_function(1.0, 1, 2)
    assert F_s == pytest.approx(1.0)
    
    # Large number of cycles
    F_s = _fluctuation_function(1000.0, 100, 2)
    assert F_s == pytest.approx(np.sqrt(10), rel=1e-10)
...
def test_dfa_1d_with_known_h(sample_signals, h):
    ...
    s_vals, F_vals = dfa(sig, degree=2, processes=1)
    ...
    # Check that F(s) increases with s (for positive H)
    if h > 0.5:
        assert np.all(np.diff(F_vals) > 0) or np.all(np.diff(F_vals) >= -1e-10)
...
def test_dfa_1d_parallel(sample_signals, h):
    ...
    s_vals, F_vals = dfa(sig, degree=2, processes=2)
...
def test_dfa_2d_input():
    ...
    s_vals, F_vals = dfa(data, degree=2, processes=1)
    
    # For 2D input: s is 1D array, F_s is 2D array
    assert isinstance(s_vals, np.ndarray)
    assert isinstance(F_vals, np.ndarray)
    assert F_vals.ndim == 2
    assert F_vals.shape[0] == len(TEST_H_VALUES)
    assert F_vals.shape[1] == len(s_vals)
...
def test_dfa_2d_parallel():
    ...
    s_vals, F_vals = dfa(data, degree=2, processes=2)
...
def test_dfa_different_degrees():
    ...
    s1, F1 = dfa(data, degree=1, processes=1)
    s2, F2 = dfa(data, degree=2, processes=1)
    s3, F3 = dfa(data, degree=3, processes=1)
...
def test_dfa_short_input():
    ...
    s_vals, F_vals = dfa(data, degree=2)
